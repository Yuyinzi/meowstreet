...
class FakeRepository:
    ...
    def record_search_attempt(self, con, attempt):
        self.attempts.append(dict(attempt))

    def record_search_results(self, con, job_id, attempt_id, results):
        rows = []
        for row in results:
            saved = dict(row)
            rows.append(saved)
...
def test_discovery_allocates_job_local_ids_and_persists_each_attempt_before_fallback():
    ...
    assert [attempt["outcome"] for attempt in repo.attempts][:2] == ["empty_results", "accepted"]
...
def test_discovery_manual_override_records_provenance_and_unsafe_override_rejected():
    ...
    assert by_type["press_releases"]["status"] == "accepted"
