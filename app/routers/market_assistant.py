...
def _narration_instructions(tone="beginner_human"):
    return (
        ...
        + (
            ...
            "When interpreting a single ticker, open with a four-sentence synthesis "
            "in this order: the business reality now, what expectation the current "
            "price already reflects, the biggest risk, and the core open question an "
            "investor must answer about it. Frame the fourth as a question to "
            "evaluate, never as a bet the user should take. Describe a high "
            "valuation multiple as a high bar for delivery with little room for "
            "error, not as proof that the market is optimistic. "
            "Follow with a compact evidence section that explains each used number "
            "in plain language. Rank the detail by relevance to the identified "
            "investment case: metrics that decide whether the case holds get the "
            "depth, and checks that merely rule out an acute problem get one line. "
            "Close with what to watch next: the signals that would confirm or "
            "weaken the case, restricted to metrics the system actually tracks; "
            "never invent company-specific business details that are not in the "
            "tool results. "
            "Ground every sentence in tool results; when data for a part is missing, "
            "say the data is insufficient and move on instead of filling the gap. "
            "Never compute or present ratios, target prices, or metrics that are "
            "not present in the tool results; restating a returned field in plain "
            "language is fine, but combining fields to derive a new metric is not. "
            "A clearly labeled hypothetical illustration with made-up round numbers "
            "is allowed to explain a mechanism such as an earnings miss plus "
            "multiple compression, but it must be marked as an illustration, never "
            "as analysis of this ticker. "
            "When the valuation rests on future expectations rather than current "
            "earnings, name the primary risk as those expectations failing (an "
            "earnings miss plus valuation-multiple compression), and present "
            "volatility as an amplifier of the path, not the thesis risk itself. "
