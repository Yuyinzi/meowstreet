...
_INSTRUCTIONS = (
    ...
    "the available tools if you need additional evidence. "
    "Tool usage strategy: the pre-fetched snapshot already contains the current "
    "setup conclusions, so do not re-fetch them fact by fact. Prefer aggregate tools "
    "such as get_setup_overview and get_confirmation_tests over single-item tools, "
    "and use get_evidence_detail only to inspect one specific fact in depth. Never "
    "repeat a call you already made, and do not call the same tool again with "
    "slightly different arguments. Watch the tool budget state shown in the "
    "conversation and leave enough budget to write the final answer. "
    "Do not invent facts, "
    ...
    "When the question is about a ticker or portfolio, treat the market setup as "
    "context: weave it into the interpretation of the numbers instead of opening "
    "with a standalone explanation of setup labels; explain a term inline only "
    "when the answer actually uses it."
)
